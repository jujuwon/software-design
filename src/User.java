...
public class User {
    ...
    public String getStudno(UserData userData){//유저의 학번을 가져오는 메소드
        ...
        return everyTimeDB.startQuery(query);//클래스에서 리스트로 반환 하지만 스트링으로 반환으로 변경
    }
    public Boolean searchUserStudnoChatRoom(String studno){//유저의 학번에 해당하는 채팅방을 찾는 메소드
        ArrayList<String> createdChatRoom = new ArrayList<String>();
        int totalElements = createdChatRoom.size();
        String new_studno = "studno." + studno.substring(0,3);
        for(int i = 0; i < totalElements; i++){
    ...
    public String matchStudno(){//유저를 학번 채팅방에 매칭하는 메소드
        userData.matchStudnoResult = studno.substring(0,3);
    ...
    public String getMatchStudnoResult(UserData userData){//유저의 매칭된 학번 채팅방이 있으면 그 채팅방을 가져오는 메소드
        String query = "SELECT matchStudnoResult FROM user WHERE matchStudnoResult = '" + userData.studno.substring(0,3) + "';";
        return db.startQuery(query);
    }
...
//-------------------------------------------------------------U4 채팅방 매칭----------------------------------------------------------------------------------
    public void matchingChatRoom(){//채팅방 매칭
        ...
        if(enterType == 0){                                          //U4.Sequence 학과 채팅방 선택 시
            ...
            if(searchResult == true){                                //U4.Sequence 채팅방이 생성되어 있을 경우
            ...
            else if(searchResult == false){                          //U4.Sequence 채팅방이 생성되어 있지 않을 경우
                showError();                                         //U4.Sequence 12 오류를 표시
            }
        }
        else if(enterType == 1){                                     //U4.Sequence 학번 채팅방 선택 시
...
//-------------------------------------------------------------U5 채팅방 입장---------------------------------------------------------------------------------------------------
    public void enterChatRoom(){//채팅방 입장
        ...
        if((enterType == 0 && matchDeptResult != null) || (enterType == 1 && matchStudnoResult != null)){ //매칭결과가 존재해야 채팅방 입장가능 추가
            if(userData.isLocked == true){                           //U5.Sequence 비밀번호가 설정되어 있을 경우
                ...
                if(isCorrect == true){                               //U5.Sequence 비밀번호가 일치할 경우
                    System.out.println("채팅방에 입장했습니다.");   //U5.Sequence 13 채팅방 표시
                }
                else if(isCorrect == false){                         //U5.Sequence 비밀번호가 일치하지 않을 경우
                    showError();                                     //U5.Sequence 14 오류 표시
                }
            }
            else if(userData.isLocked == false){                     //U5.Sequence 비밀번호가 설정되어 있지 않을 경우
                System.out.println("채팅방에 입장했습니다.");       //U5.Sequence 13 채팅방 표시
            }
        }
    }
...
//-------------------------------------------------------------U8 채팅방 잠금 설정---------------------------------------------------------------------------------------------------
    public void lockChatRoom(){//채팅방 잠금 설정                     //U8.Sequence 1 잠금 설정
        ...
        if(isCorrect == true){                                       //U8.Sequence 비밀번호가 일치하는 경우
